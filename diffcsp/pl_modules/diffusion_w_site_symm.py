    ...
    for (sym, frac_coord, atm_type) in zip(site_symm_argmax, frac_coords, atom_types):
        ...
        for wp, binary in wp_to_binary.items():
            if np.sum(np.equal(binary, sym)) == sym.shape[-1]:
                ...
                closes.append((close, wp, np.linalg.norm(close - frac_coord)))
        ...
        try:
            ...
            for index in range(len(wyckoff)): 
                new_frac_coords.append(wyckoff[index].operate(frac_coord))
...
class CSPDiffusion(BaseModule):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.decoder = hydra.utils.instantiate(self.hparams.decoder, latent_dim = self.hparams.latent_dim + self.hparams.time_dim, pred_type = True, site_symm_type=True, smooth = True)
    ...
    def forward(self, batch):
        ...
        pred_lattice, pred_x, pred_t, pred_symm = self.decoder(time_emb, atom_type_probs, input_frac_coords, lattice_feats, input_lattice, batch.num_atoms, batch.batch, site_symm_probs=site_symm_probs)
    ...
    def sample(self, batch, diff_ratio = 1.0, step_lr = 1e-5):
        ...
        for t in tqdm(range(self.beta_scheduler.timesteps, 0, -1)):
            ...
            pred_l, pred_x, pred_t, pred_symm = self.decoder(time_emb, t_t, x_t, lattice_feats_t, l_t, batch.num_atoms, batch.batch, site_symm_probs=symm_t)
            ...
            pred_lattice, pred_x, pred_t, pred_symm = self.decoder(time_emb, t_t_minus_05, x_t_minus_05, lattice_feats_t_minus_05, l_t_minus_05, batch.num_atoms, batch.batch, site_symm_probs=symm_t_minus_05)
        ...
        breakpoint()
        traj[0] = modify_frac_coords(traj[0], batch.spacegroup, batch.num_atoms)
